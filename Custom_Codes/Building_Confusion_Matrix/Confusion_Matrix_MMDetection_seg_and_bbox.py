...
config_file = '/home/data/files/Yaman/Satalite_Images/Models_Training/MMDetection/Aug_Original/mask_rcnn_custom.py'
checkpoint_file = '/home/data/files/Yaman/Satalite_Images/Models_Training/MMDetection/Aug_Original/mask_rcnn_custom_V2/epoch_20.pth'
model = init_detector(config_file, checkpoint_file, device='cuda:0')


coco_gt = COCO('Aug_Original/test/_annotations.coco.json')
...
def conf_matrix_calc_mm(gt_annos, pred_boxes, pred_labels, pred_scores, n_classes, conf_thresh, iou_thresh):
    ...
    if len(gt_bboxes) > 0 and len(pred_boxes) > 0:
        ious = bbox_overlaps(torch.tensor(pred_boxes, dtype=torch.float32),
...
def conf_matrix_calc_seg(gt_annos, pred_masks, pred_labels, pred_scores, image_shape, n_classes, conf_thresh, iou_thresh):
    confusion_matrix = np.zeros((n_classes + 1, n_classes + 1))

    gt_classes = np.array([ann['category_id'] for ann in gt_annos])
    gt_masks = np.array([
        coco_seg_to_mask(ann['segmentation'], image_shape[0], image_shape[1])
        for ann in gt_annos
    ])

    if len(gt_masks) == 0:
        gt_masks = np.empty((0, *image_shape), dtype=bool)
    if len(pred_masks) == 0:
        pred_masks = np.empty((0, *image_shape), dtype=bool)
        pred_labels = np.array([])
        pred_scores = np.array([])
    ...
    matched_gt = np.zeros(len(gt_masks), dtype=bool)
    matched_pred = np.zeros(len(pred_masks), dtype=bool)
...
seg_conf_matrix = np.zeros((num_classes + 1, num_classes + 1))

for img_id in image_ids:
    img_info = coco_gt.loadImgs(img_id)[0]
    img_path = os.path.join('Aug_Original/test', img_info['file_name'])
    ...
    if pred_instances is not None and len(pred_instances.bboxes) > 0:
        ...
        pred_masks = pred_instances.masks.cpu().numpy()
    else:
        pred_boxes = np.empty((0, 4))
        pred_scores = np.array([])
        pred_labels = np.array([])
        pred_masks = np.empty((0, height, width), dtype=bool)
    ...
    seg_conf_matrix += conf_matrix_calc_seg(
        gt_annos, pred_masks, pred_labels, pred_scores,
        (height, width), num_classes, conf_thresh=0.25, iou_thresh=0.7
    )


class_names_with_bg = class_names + ['Background']
df_det = pd.DataFrame(conf_matrix.astype(int), index=class_names_with_bg, columns=class_names_with_bg)
df_seg = pd.DataFrame(seg_conf_matrix.astype(int), index=class_names_with_bg, columns=class_names_with_bg)
